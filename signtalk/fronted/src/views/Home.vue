...
const handleSubmit = () => {
  ...
  const apiUrl = isLogin.value ? API_ENDPOINTS.login : API_ENDPOINTS.register;
  const dataToSend = {
  ...
  axios.post(apiUrl, dataToSend)
    ...
    .catch(error => {
      ...
      if (error.response && error.response.data) {
        // 如果后端返回了 message 字段，直接使用
        if (error.response.data.message) {
          alert(error.response.data.message);
        } 
        // 如果是序列化器错误（字段验证错误），格式化为可读字符串
        else if (typeof error.response.data === 'object') {
          const errorMessages = [];
          for (const key in error.response.data) {
            const messages = Array.isArray(error.response.data[key]) 
          ...
          alert(error.response.data || '注册失败，请检查输入信息');
        }
      } else {
        alert('服务器连接失败');
      }
